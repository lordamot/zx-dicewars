...
| `#6000` | **CODE block start** = entry point (`start:` in main.asm). Loaded from disk (`dicewars`<C>, ~21K) |
| ... | game code, initialised vars, font (`font8`, `dicefont`), music data (`ay_note_table`, `song_a/b/c`), the two endgame photo screens (`scr_youwin`, `scr_gameover`, 6912 bytes each) |
...
The BASIC loader does `CLEAR 24575`, loads `screen`<C> straight to
#4000 (the loading screen, showing while the rest loads), loads the game
CODE at 24576 and `RANDOMIZE USR 24576`; `start:` immediately takes over
SP and interrupts, so nothing else depends on BASIC's state.
